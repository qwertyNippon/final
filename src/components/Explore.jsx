            ...
            {anime.posts && anime.posts.length > 0 ? Object.values(anime.posts).map((a, i) => {
                return <div key={i} className="cardEx  flx-c mx">
                    <div className='cardB white flx-c p8 tog '>
                        <div className="flx1">{a.rating}</div>
                        <hr className="h" />
                        <div className="flx1"><Link target='_blank' to={a.trail}>Watch Trailer</Link></div>
                        <hr className="h" />
                        <div className="flx1"><Link target='_blank' to={a.new_url}>Anime Info</Link></div>
                        <hr className="h" />
                        <div className="flx8 scrol">{a.syn}</div>
                        <div className="flx1"><button>Add to Watch List</button></div>
                    </div>
